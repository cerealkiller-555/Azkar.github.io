...
const SettingsSection = ({
    ...
    <div className="animate-slide-up space-y-8">
        {/* PWA Install */}
        {deferredPrompt && (
            <div className="p-8 bg-gradient-to-br from-[#423E87] to-[#2E2A5E] rounded-3xl shadow-2xl border border-white/10 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-accent opacity-10 blur-3xl translate-x-16 -translate-y-16" />
                ...
                <button onClick={installPWA} className="w-full py-4 rounded-2xl bg-accent text-primary font-black hover:bg-accent-light transition-all active:scale-95 shadow-xl shadow-black/20">
        ...
        <div className="glass-panel p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest ml-1">{t.cityLabel}</label>
                    <input
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest ml-1">{t.countryLabel}</label>
                    <input
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass-panel p-8 space-y-6">
                ...
                <button onClick={toggleDarkMode} className="w-full flex items-center justify-between p-5 rounded-2xl bg-bg-subtle dark:bg-slate-900 border-2 border-glass-border hover:border-accent transition-all">
            ...
            <div className="glass-panel p-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setLanguage("ar")} className={`py-4 rounded-2xl text-sm font-black border-2 transition-all ${language === "ar" ? "bg-primary text-white border-primary shadow-lg shadow-primary/20" : "bg-bg-subtle dark:bg-slate-900 text-text-secondary border-glass-border hover:border-accent"}`}>
                        العربية
                    </button>
                    <button onClick={() => setLanguage("en")} className={`py-4 rounded-2xl text-sm font-black border-2 transition-all ${language === "en" ? "bg-primary text-white border-primary shadow-lg shadow-primary/20" : "bg-bg-subtle dark:bg-slate-900 text-text-secondary border-glass-border hover:border-accent"}`}>
                        English
                    </button>
                </div>
            </div>
        ...
        <div className="glass-panel p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <input type="email" value={userProfile.email} readOnly className="w-full px-5 py-4 rounded-2xl bg-bg-subtle/50 dark:bg-slate-900/50 border-2 border-glass-border text-text-tertiary font-bold cursor-not-allowed" />
                </div>
            </div>
        </div>

        {/* Reset */}
        <button onClick={resetAllProgress} className="w-full py-5 rounded-3xl bg-rose-500 hover:bg-rose-600 text-white font-black shadow-xl shadow-rose-500/20 transform active:scale-[0.98] transition-all flex items-center justify-center gap-3">
